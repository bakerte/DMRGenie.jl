...
%DIF DEL file_for_diff.tex   Tue Jun 23 17:11:19 2026
%DIF ADD paper.tex           Tue Jun 23 17:11:16 2026
...
\graphicspath{{FIGs/}}
 %DIF > 
\usepackage{xcolor} % Enables advanced color management %DIF > 
\usepackage{hyperref} % Enables hyperlinks %DIF > 
 %DIF > 
\hypersetup{ %DIF > 
    colorlinks=true,  % Switches from boxes to colored text %DIF > 
    linkcolor=blue,   % Color for internal links (sections, figures, TOC) %DIF > 
    citecolor=red,  % Color for bibliographical citations %DIF > 
    urlcolor=cyan,    % Color for external web URLs %DIF > 
    filecolor=magenta % Color for local files %DIF > 
} %DIF > 
...
In any scientific endeavour, the need to use numerical techniques is highly necessary. And for new advances to be made, new methods must be able to be implemented into any given language to evolve beyond the state-of-the-art. For anyone developing new algorithms, the need for a programming framework that enables as many solutions of the two-language problem as possible is necessary, making Julia a strong contender for algorithmic development.

Quantum information has become a tantalizing path to creating new algorithms and methods. These range from quantum algorithms implemented on quantum computers \cite{nielsen2010quantum} to entanglement renormalization techniques. \DIFdelbegin \footnote{\DIFdel{This class of methods is more commonly referred to as tensor networks, simply, but with the rise of machine-learning and other methods particularly in quantum chemistry that are formulated on tensor networks, a distinguishing name is required. We choose to expand the usage of the term from Ref.~\mbox{%DIFAUXCMD
\cite{vidal2007entanglement} }\hskip0pt%DIFAUXCMD
since all methods are fundamentally optimizing over quantities relevant for quantum information (}%DIFDELCMD < {\it %%%
\DIFdel{i.e.}%DIFDELCMD < }%%%
\DIFdel{, the entanglement) and renormalization was originally introduced to describe this class of methods \mbox{%DIFAUXCMD
\cite{wilson1983renormalization,krishna1980renormalization}}\hskip0pt%DIFAUXCMD
. The methods required to implement an algorithm in this context is different from other uses, notably machine learning.}} 
%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{This class of methods is more commonly referred to as tensor networks, simply, but with the rise of machine-learning and other methods particularly in quantum chemistry that are formulated on tensor networks, a distinguishing name is required. We choose to expand the usage of the term from Ref.~\mbox{%DIFAUXCMD
\cite{vidal2007entanglement} }\hskip0pt%DIFAUXCMD
since all methods are fundamentally optimizing over quantities relevant for quantum information (}{\it \DIFadd{i.e.}}\DIFadd{, the entanglement) and renormalization was originally introduced to describe this class of methods \mbox{%DIFAUXCMD
\cite{wilson1983renormalization,krishna1980renormalization}}\hskip0pt%DIFAUXCMD
. The methods required to implement an algorithm in this context is different from other uses, notably machine learning.
}\DIFaddend 
...
The use of the diagram notation is useful for representing large numbers of tensors in a coherent way without representing the full mathematical detail (see Fig.~\ref{network}). For example, the wavefunction when decomposed through a series of reshapes and singular value decompositions (SVD) takes the form of a matrix product state (MPS) \cite{bakerCJP21}\DIFaddbegin \DIFadd{.
...
\label{mpsform}
\end{align}
which is far more intricate than the diagram notation but conveys no extra information. Any quantum operator (for example, the Hamiltonian) can be decomposed into a similar site-by-site form, called a matrix product operator (MPO).

Building these algorithms from a purely diagrammatic level would be useful. This opens the possibility to using a graphical user interface to denote the various steps in the implementation of the entanglement renormalization algorithms.
...
We will first discuss the background software (DMRjulia \DIFdelbegin \DIFdel{and TENPACK}\DIFdelend \DIFaddbegin \url{https://github.com/bakerte/DMRJtensor.jl} \DIFadd{and TENPACK }\url{https://github.com/bakerte/TensorPACK.jl}\DIFaddend ; available in Julia's package system as {\tt DMRJtensor} and {\tt TensorPACK}) originally developed in 2016 and first introduced to Github in 2020 and full debut on the Julia package environment in 2021. This means that the library has spanned many version of the Julia version starting originally in v0.5 and now continuing to the present version (v1.11 at time of submission) and beyond. We believe that some choices from our implementation may be interesting to the broader community, although many of the improvements that were introduced here also independently appeared in Julia from other groups, showing that common issues are being encountered and handled in similar ways.

DMRGenie \DIFaddbegin \DIFadd{(located at }\url{https://dmrgenie.rs.uvic.ca/}\DIFadd{) }\DIFaddend is the natural extension of the previously developed software which we debut here. This builds on top of DMRjulia to include a graphical interface for easy development of new algorithms. This is made publicly available through a web interface and available for use with some limitations on the maximum size of the tensors and enforce reasonable computational requirements \cite{dmrgenie}.
...
\DIFaddbegin \section{\DIFadd{Quantum symmetries}}
\label{qsymmetries}

\DIFadd{TENPACK currently supports local $U(1)$ and $\mathbb{Z}_n$ symmetries. The basic idea is to establish the relevant quantum numbers on each physical index (drawn as the vertical index on the MPS). Specifying the quantum numbers on each of the physical indices is sufficient to determine the quantum numbers on all other indices, provided that the total flux of the input Hamiltonian is a net zero (}{\it \DIFadd{i.e.}}\DIFadd{, symmetry-preserving Hamiltonian). Given this case, all tensors exhibit a block structure, where each block has a set of non-zero values and no non-zero values are available between quantum number sectors.
}

\DIFadd{Given a symmetry preserving Hamiltonian, one can use rules similar to Kirchoff's loop rules (with total flux equal to zero) to determine the quantum numbers on each index. All of this is automatically handled in the quantum number tensor implementation in TENPACK for abelian quantum numbers. 
}

\DIFadd{For example, consider the Hamiltonian for two spin-half objects on two different sites
}\begin{equation}
\DIFadd{\mathbf{S}_1\cdot\mathbf{S}_2=\frac{1}{4}\left(\begin{array}{cccc}
1 & 0 & 0 & 0\\
0 & -1 & 2 & 0\\
0 & 2 & -1 & 0\\
0 & 0 & 0 & 1\\
...
\hat H = -J\sum_{i} \left( \hat S^x_i \hat S^x_{i+1} + \hat S^y_i \hat S^y_{i+1} + \hat S^z_i \hat S^z_{i+1}\right)
\end{equation}
...
\DIFdelbegin \DIFdel{Details }\DIFdelend \DIFaddbegin \DIFadd{For details }\DIFaddend of the symmetries and how they need to be implemented \DIFdelbegin \DIFdel{are included in Appendix}\DIFdelend \DIFaddbegin \DIFadd{see}\DIFaddend ~\ref{qsymmetries}.
...
The Tensor Network Builder is a drag and drop interface to build tensor networks. The rank of any tensor is specified before they are dropped onto the canvas. The number of connections points on a tensor is equal to the rank of the tensor. Indices can be created by dragging from a connection point on a tensor. It is possible to specify the name and dimension of all indices, and to rename an index if needed. Possible operations on tensors include contractions (see example of interface in Fig.~\ref{GUIcontraction}), singular value decompositions, eigenvalue decompositions, QR decompositions, and LQ decompositions. When the user is done, the "submit" button can be clicked and the algorithm will be run on the backend. The output of the algorithm will be the tensors that resulted from the last operation and will be given as a downloadable file.

ReactJS is used for the \DIFdelbegin \DIFdel{tensor network builder }\DIFdelend \DIFaddbegin \DIFadd{Tensor Network Builder }\DIFaddend because of component reusability and the fast rendering. The ReactFlow package was designed to build node-based editors and interactive diagrams; however, graphs in computer science have similar structures to tensor networks. The nodes and edges in ReactFlow were generalized to tensors and indices. 

\DIFaddbegin \DIFadd{For details on how to use the Tensor Network Builder, see Appendix~\ref{builder}.
}
...
\DIFaddbegin \section{\DIFadd{Installation and usage}}

\DIFadd{The basic libraries for DMRjulia and TENPACK can be obtained from online repositories \mbox{%DIFAUXCMD
\cite{dmrjulia,tenpack} }\hskip0pt%DIFAUXCMD
or from the Julia package environment:
}\DIFmodbegin
\begin{lstlisting}[language = Julia,alsolanguage=DIFcode]
%DIF > julia> ]
%DIF > pkg> add DMRGenie DMRJtensor TensorPACK
\end{lstlisting}
\DIFmodend

\DIFadd{The DMRGenie GUI can be used online or locally by cloning the github repository~\mbox{%DIFAUXCMD
\cite{dmrgenie}}\hskip0pt%DIFAUXCMD
. Note that if DMRGenie is used locally, then julia version $1.9.x$ is required.
}

\subsection{\DIFadd{Setting Up and Running DMRGenie}}

\DIFadd{To set up and run DMRGenie, one can simply navigate to }{\tt \DIFadd{DMRGenie.jl/DMRGenie}} \DIFadd{then run the script }{\tt \DIFadd{run.sh}} \DIFadd{from the command line.
}

\DIFadd{The actual steps being taken are as follows:
}
...
%DIF > Pkg.instantiate()
%DIF > Pkg.activate(".")
%DIF > julia> using Genie
%DIF > julia> Genie.loadapp()
\end{lstlisting}
\DIFmodend

\DIFadd{Then one may open the website locally by navigating to }{\tt \DIFadd{http://127.0.0.1:8000}} \DIFadd{in their browser.
}



\DIFaddend \section{Deployment}
...
\DIFdelbegin \section{\DIFdel{Installation and usage}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The basic libraries for DMRjulia and TENPACK can be obtained from online repositories \mbox{%DIFAUXCMD
\cite{dmrjulia,tenpack} }\hskip0pt%DIFAUXCMD
or from the Julia package environment:
}%DIFDELCMD < \begin{lstlisting}[language = Julia]
%DIFDELCMD < %%%
%DIFAUXCMD NEXT
\DIFmodbegin
\begin{lstlisting}[language = Julia,alsolanguage=DIFcode]
...
%DIFDELCMD < %%%
\DIFdel{The DMRGenie GUI can be used online or locally by cloning the github repository~\mbox{%DIFAUXCMD
\cite{dmrgenie}}\hskip0pt%DIFAUXCMD
. Note that if DMRGenie is used locally, then julia version $1.9.x$ is required.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section{Acknowledgement}
...
\section{\DIFdelbegin \DIFdel{Running locally}\DIFdelend \DIFaddbegin \DIFadd{Tensor Network Builder guide}\DIFaddend }\DIFaddbegin \label{builder}
\DIFaddend 

\DIFdelbegin \DIFdel{DMRGenie can be run locally and we show how to do this here.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Setup}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Navigate to the `tensez` directory once the repository has been cloned and then into the `DMRGenie` folder within it.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Install the necessary packages with instantiate.
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Running the App}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The app must be activated by running the following from the `DMRGenie` directory
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\section{\DIFdel{Quantum symmetries}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{qsymmetries}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{TENPACK currently supports local $U(1)$ and $\mathbb{Z}_n$ symmetries. The basic idea is to establish the relevant quantum numbers on each physical index (drawn as the vertical index on the MPS). Specifying the quantum numbers on each of the physical indices is sufficient to determine the quantum numbers on all other indices, provided that the total flux of the input Hamiltonian is a net zero (}%DIFDELCMD < {\it %%%
\DIFdel{i.e.}%DIFDELCMD < }%%%
\DIFdel{, symmetry-preserving Hamiltonian). Given this case, all tensors exhibit a block structure, where each block has a set of non-zero values and no non-zero values are available between quantum number sectors.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Given a symmetry preserving Hamiltonian, one can use rules similar to Kirchoff's loop rules (with total flux equal to zero) to determine the quantum numbers on each index. All of this is automatically handled in the quantum number tensor implementation in TENPACK for abelian quantum numbers. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{For example, consider the Hamiltonian for two spin-half objects on two different sites
}\begin{displaymath}
\DIFdel{\mathbf{S}_1\cdot\mathbf{S}_2=\frac{1}{4}\left(\begin{array}{cccc}
1 & 0 & 0 & 0\\
0 & -1 & 2 & 0\\
0 & 2 & -1 & 0\\
0 & 0 & 0 & 1\\
...
\DIFdel{where $\mathbf{S}=\boldsymbol{\sigma}/2$ for the Pauli vector $\boldsymbol{\sigma}$ \mbox{%DIFAUXCMD
\cite{townsend2000modern} }\hskip0pt%DIFAUXCMD
and subscripts denote which site the spin vector is located. The matrix can be divided into three blocks, and in general, we can write a block structure of completely independent pieces as
}\begin{displaymath}
\DIFdel{\mathbf{S}_1\cdot\mathbf{S}_2=1\oplus\left(\begin{array}{cc}
-1 & 2 \\
2 & -1
\end{array}\right)\oplus1
}\end{displaymath}%DIFAUXCMD
...
The network canvas is where the tensor network is displayed. The current canvas is provided by the ReactFlow package. The network canvas is a \textbf{div} element inside the $<$\textbf{ReactFlow.ReactFlowProvider}$>$ element and is where the tensors are displayed, moved, and edited.
...
Since IDs must be unique, if an edge is created then it's ID is `e\$\{eId\}` where \textbf{eId} is incremented every time an invisible edge is created. If an edge is deleted and it connected to an invisible node then the invisible node will be deleted as well. This is done by the \textbf{onEdgesDelete} function.
...
The SVD composition accepts the ID of a single tensor and two groups of the tensors's indices. The decomposition will result in three tensors. The tensors will be two complex unitary matrices and a diagonal matrix. This is done by replacing the original tensor by these three tensors. The tensor that results from the first group of indices will be connected to the tensors that the original edges were connected to (excluding the original tensor) and the diagonal matrix. The same applies to the tensor produced by the second group of indices. The diagonal matrix is connected to the unitary matrices.
...
The QR/LQ decomposition accepts the same input as the SVD decomposition. The resulting tensors will be the Q(L) and R(Q) matrices if the QR(LQ) decomposition is done.  The tensor that results from the first group of indices will be connected to the tensors that the original edges were connected to (excluding the original tensor). The same applies to the tensor produced by the second group of indices.

An error will appear if the original tensor does not exist or the indices are not grouped properly.


\subsection{Eigenvalue Decomposition}
The eigenvalue decomposition accepts the ID of a single tensor and returns three square matrices. These matrices are the diagonal matrix of eigenvalues, the matrix of eigenvectors, and the matrix of inverse eigenvectors. This is done by replacing the original matrix with these square matrices and connecting them (in the order of eigenvectors-eigenvalues-inverse eigenvectors). 
...
\textbf{\DIFdelbegin \DIFdel{invisible}\DIFdelend \DIFaddbegin \DIFadd{Invisible}\DIFaddend } nodes are \textbf{ALWAYS} the target node, never the source. Changing this will cause some errors in the code.

\subsection{Additional Info}
Multiple elements can be selected by \textbf{holding command+clicking}. React elements must start with a capital letter or errors may occur (this is a convention but it is easy to forget when learning)


%Make sure that the path ends with `tensez/DMRGenie`, if not then run `{\tt cd("<path\_to\_repo>/tensez/DMRGenie")}` from the Julia CLI.
%
