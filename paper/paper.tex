...
We discuss implementation of a graphical user interface for tensor operations in scientific simulations related to entanglement renormalization. This software, which we call DMRGenie, based on its heavy reliance on Genie which is a well-known software package for web applications. We also discuss implementation of the software onto the Digital Research Alliance for permanent web-hosting. Scientific simulations with tensor network-based framework of quantum algorithm methods is now possible through this tool without length start-up or learning times.
...
On one hand, for ideas to be implemented into computer code, which creates a need for easy-to-use high-level languages that are easy to code. On the other hand, scientific programming often requires runs on large systems, implying that performance is a necessity. To accomplish this second demand, a low-level language is required to control as much as possible in an implementation of an algorithm. This split between needs for different programming projects is known as the two-language problem.
...
In any scientific endeavour, the need to use numerical techniques is highly necessary. And for new advances to be made, new methods must be able to be implemented into any given language to evolve beyond the state-of-the-art. For any algorithm development team, the need for custom software that displays as many solutions of the two-language problem is necessary, making Julia a strong option for use in algorithm development.
...
Implementing algorithms from a purely diagrammatic level is a desired possibility, provided that a proper implementation of a graphical interface to implement steps in the implementation of the entanglement renormalization algorithms. It ultimately allows us to construct a simple graphical interface for computations without much code and without much math.

Julia's solution of the two-level problem allows for an easy implementation of algorithms and simultaneous fast execution of scientific code. We discuss in this article efforts to construct a fully working scientific software package for entanglement renormalization that is simultaneously fast and lightweight. This software package was then extended to include a user interface that allows for the graphical construction of algorithms without any coding background, and this is our principle goal here.
...
DMRGenie is the natural extension of the previously developed software which we debut here. This builds on top of DMRjulia to include a graphical interface for easy development of new algorithms. This is hosted natively on a website and available for use with some limitations on the maximum size of the tensors to reduce server traffic \cite{dmrgenie}.

\section{Entanglement renormalization with DMRjulia}

DMRjulia is a software package in the Julia package environment that is 100\% written in the Julia programming language. This makes the software fast and light-weight, ultimately taking on a few kilobytes to install, and optimized to the point of being as fast as lower-level implementations of the same algorithms.
...
As a quirk of the development of Julia, the issue of type stability was of paramount importance to ensure that any code developed in the language avoids generating too much overhead because of Julia's multiple dispatch capabilities. Because the compiler in Julia must assume that any possible input can be entered into a function, the assumptions of how types are converted and exactly what type is created can generate excess allocations which ultimately slow a program. Eliminating these excess allocations is necessary to create efficient code that is competitive with lower level languages.

For example, when reshaping a tensor, the rank of the tensor changes. The implementation of the basic {\tt Array} type in Julia contains both the element data type and also the rank of the tensor. Because the rank of the tensor in entanglement renormalization methods, this naturally creates issues with the implementation of Julia's desire to reduce allocations for greater efficiency.

A bridge between operations on tensors and the BLAS  functions that Julia calls was created as a stand-alone dependency (still 100\% written in Julia with no dependencies beyond the basic Julia implementation) called Tensor (Linear Algebra) Package (TENPACK) was created to interface between basic Julia and DMRjulia. This effectively replaces {\tt LinearAlgebra} and creates many functions with a similar name (and similar functionality) to {\tt LinearAlgebra} but for tensor inputs.
...
In order to obtain the basic library for DMRjulia and TENPACK, these can be obtained by an online repository \cite{dmrjulia,tenpack} or from the Julia package environment:
...
TENPACK currently supports local $U(1)$ and $\mathbb{Z}_n$ symmetries. The basic idea is to establish the relevant quantum numbers on each physical index (drawn as the vertical index on the MPS). Specifying the quantum numbers on each of the physical indices is sufficient to determine the quantum numbers on all other indices, provided that the total flux of the input Hamiltonian is a net zero ({\it i.e.}, symmetry-preserving Hamiltonian). Given this case, all tensor exhibit a block structure, where each block has a set of non-zero values and no non-zero values are available between quantum number sectors.
