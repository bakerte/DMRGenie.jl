...
\usepackage{xcolor} % Enables advanced color management
\usepackage{hyperref} % Enables hyperlinks

\hypersetup{
    colorlinks=true,  % Switches from boxes to colored text
    linkcolor=blue,   % Color for internal links (sections, figures, TOC)
    citecolor=red,  % Color for bibliographical citations
    urlcolor=cyan,    % Color for external web URLs
    filecolor=magenta % Color for local files
}
...
For details of the symmetries and how they need to be implemented see~\ref{qsymmetries}.
...
For details on how to use the Tensor Network Builder, see Appendix~\ref{builder}.
...
The DMRGenie GUI can be used online or locally by cloning the github repository~\cite{dmrgenie}. Note that if DMRGenie is used locally, then julia version $1.9.x$ is required.

\subsection{Setting Up and Running DMRGenie}

To set up and run DMRGenie, one can simply navigate to {\tt DMRGenie.jl/DMRGenie} then run the script {\tt run.sh} from the command line.

The actual steps being taken are as follows:
...
Then one may open the website locally by navigating to {\tt http://127.0.0.1:8000} in their browser.
...
\section{Tensor Network Builder guide}\label{builder}
...
\textbf{Invisible} nodes are \textbf{ALWAYS} the target node, never the source. Changing this will cause some errors in the code.
